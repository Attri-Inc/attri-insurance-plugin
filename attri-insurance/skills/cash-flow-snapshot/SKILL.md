...
bands based on each policyholder's historical payment variance, and flags specific
risks by name.
...
**From QuickBooks:**
- AR aging report: policyholder name, invoice amount, invoice date, due date, days outstanding
...
For each AR policyholder (or income source from CSV), calculate:
...
If fewer than 3 payments exist for a policyholder, use the population mean as the
...
- **Late-payer risk:** "Policyholder X historically pays 18 days late; that shifts
...
- **Thin data warning:** "Only 2 payments on record for Policyholder Y — confidence
